...
namespace TownOfUs.ImpostorRoles.ExploderMod
{
    ...
    public class PerformExplodeKill
    {
        ...
        {
            var flag = PlayerControl.LocalPlayer.Is(RoleEnum.Exploder);
            if (!flag) return true;
            ...
            if (role.ExplodeTimer() > 0) return false;
            if (role.Enabled == true) return false;
            if (role.Player.inVent)
            {
                role.ExplodeButton.SetCoolDown(0.01f, 1f);
                return false;
            }
            ...
            if (role.ClosestPlayer.IsOnAlert())
            {
                if (role.ClosestPlayer.IsShielded())
                {
                    ...
                    if (CustomGameOptions.ShieldBreaks) role.LastExploded = DateTime.UtcNow;
                    role.ExplodeButton.SetCoolDown(0.01f, 1f);
                ...
                else if (role.Player.IsShielded())
                {
                    ...
                    {
                        role.ExplodedPlayer = target;
                        role.ExplodeButton.SetTarget(null);
                        DestroyableSingleton<HudManager>.Instance.KillButton.SetTarget(null);

                        role.TimeRemaining = CustomGameOptions.ExplodeDuration;
                        ...
                        var writer4 = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId,
                            (byte)CustomRPC.Explode,
                        SendOption.Reliable, -1);
                        writer4.Write(PlayerControl.LocalPlayer.PlayerId);
                        writer4.Write(role.ExplodedPlayer.PlayerId);
                ...
                else
                {
                    ...
                    if (CustomGameOptions.KilledOnAlert && !target.IsProtected())
                    {
                        ...
                        writer3.Write(role.ExplodedPlayer.PlayerId);
                        AmongUsClient.Instance.FinishRpcImmediately(writer3);
                        // role.Player.SetKillTimer(0);
                    }
                    else
                    {
                        role.LastExploded.AddSeconds(CustomGameOptions.ProtectKCReset + 0.01f);
                        role.ExplodeButton.SetCoolDown(0.01f, 1f);
                    }
                }
                return false;
            ...
            {
                role.LastExploded.AddSeconds(CustomGameOptions.VestKCReset + 0.01f);
                role.ExplodeButton.SetCoolDown(0.01f, 1f);
                return false;
            }
            ...
            writer2.Write(role.ExplodedPlayer.PlayerId);
